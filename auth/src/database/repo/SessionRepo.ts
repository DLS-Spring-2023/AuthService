...
class SessionRepo {
     ...
     * findValidBySession
     */
    public async findValidBySessionId(session: bigint) {
        const conn = await this.db.getConnection();

        const result = await conn.query(`SELECT * FROM ${this.table}_session WHERE (session_id = ?) AND (valid = 1) LIMIT 1;`, [session]);
    ...
    public async startNewSession(session: AccountSession) {
        ...
        const query = `INSERT INTO ${this.table}_session (session_id, ${this.table}_id, expires) VALUES (?, ?, ?)`;
        const prep = [session.session_id, session.account_id, expires];
