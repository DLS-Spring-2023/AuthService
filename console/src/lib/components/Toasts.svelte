<script lang="ts">
	...
	const closeAfter = (
		_: HTMLDivElement,
		{ delay, id }: { delay: number | undefined; id: number | undefined }
	) => {
		if (delay) {
			setTimeout(() => {
				store.pop(id);
...
<div class="fixed top-20 right-10 max-w-sm flex flex-col items-end drop-shadow-md z-50">
	{#each toasts as toast, index (toast.id)}
		<div
			...
			use:closeAfter={{ delay: toast.closeAfter, id: toast.id }}
		>
			<div
				class="h-full w-20 rounded-l-md p-4 flex items-center text-white {getBackground(
					toast.type
				)}"
			>
			...
			<button
				on:click={() => store.pop(toast.id)}
				type="button"
				class="w-6 mr-2 h-fit bg-slate-200 text-black rounded-r-md"
			>
				<Close />
			</button>
		</div>
