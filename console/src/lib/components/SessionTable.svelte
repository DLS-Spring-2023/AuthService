...
<div class="border-full rounded-xl text-left">
	<div class="grid grid-cols-6 pt-2 items-center">
		...
		{#each sessions as session, i}
			<p
				class="grid-item text-gray-400 pl-6 {i % 2 === 0 ? 'bg-slate-800' : ''} {i ===
				sessions.length - 1
					? 'rounded-bl-xl'
					: ''}"
			>
				{#if session.browser}{session.browser}{:else}<span class="w-6"><Question /></span>{/if}
			</p>
			<p class="grid-item text-gray-400 {i % 2 === 0 ? 'bg-slate-800' : ''}">
				{#if session.os}{session.os}{:else}<span class="w-6"><Question /></span>{/if}
			</p>
			<p class="grid-item text-gray-400 {i % 2 === 0 ? 'bg-slate-800' : ''}">
				{#if session.location}{session.location}{:else}<span class="w-6 ml-1"><Question /></span
					>{/if}
			</p>
			<p class="grid-item text-gray-400 {i % 2 === 0 ? 'bg-slate-800' : ''}">
				{#if session.ip}{session.ip}{:else}<span class="w-6"><Question /></span>{/if}
			</p>
			<p class="grid-item text-gray-400 {i % 2 === 0 ? 'bg-slate-800' : ''}">
				{new Date(session.created_at).toLocaleDateString()}
			</p>
			<div
				class="grid-item {i % 2 === 0 ? 'bg-slate-800' : ''} {i === sessions.length - 1
					? 'rounded-br-xl'
					: ''}"
			>
				<form action="/killsession" method="POST" use:enhance={submit}>
					...
					<input
						type="hidden"
						name="type"
						value={$page.route.id === '/(private)/console/dashboard/sessions' ? 'account' : 'user'}
					/>
