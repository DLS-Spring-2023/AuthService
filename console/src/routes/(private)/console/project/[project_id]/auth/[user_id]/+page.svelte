    ...
    const submitBlock: SubmitFunction = () => {
        return ({result}) => {
            switch(result.type) {
                case "success":
                    invalidateAll();
                    toast.push({ type: ToastType.success, message: "User successfully blocked", closeAfter: 2000});
                    break;
                case "failure":
    ...
    const submitVerify: SubmitFunction = () => {
        return ({result}) => {
            switch(result.type) {
                case "success":
                    invalidateAll();
                    toast.push({ type: ToastType.success, message: "User successfully verified", closeAfter: 2000});
                    break;
                case "failure":
                    toast.push({ type: ToastType.error, message: result.data?.message, closeAfter: 2000});
            }
    ...
    const submitName: SubmitFunction = () => {
        return ({result}) => {
            switch(result.type) {
                case "success":
                    invalidateAll();
                    toast.push({ type: ToastType.success, message: "Name successfully updated", closeAfter: 2000});
                    break;
                case "failure":
                    toast.push({ type: ToastType.error, message: result.data?.message, closeAfter: 2000});
                    name = user.name;
    ...
    const submitEmail: SubmitFunction = () => {
        return ({result}) => {
            switch(result.type) {
                case "success":
                    invalidateAll();
                    toast.push({ type: ToastType.success, message: "Email successfully updated", closeAfter: 2000});
                    break;
                case "failure":
                    toast.push({ type: ToastType.error, message: result.data?.message, closeAfter: 2000});
                    email = user.email;
    ...
    const submitPassword: SubmitFunction = () => {
        return ({result}) => {
            switch(result.type) {
                case "success":
                    invalidateAll();
                    toast.push({ type: ToastType.success, message: "Password successfully updated", closeAfter: 2000});
                    break;
                case "failure":
                    toast.push({ type: ToastType.error, message: result.data?.message, closeAfter: 2000});
            }
...
<div class="w-full">
    ...
    <div class="bg-dark brightness-125 m-12 flex flex-col justify-between rounded-2xl border-full">
        <div class="flex justify-between items-center p-8">
            ...
            <div class="{user.enabled ? user.verified ? 'text-green-400' : 'text-gray-400' : 'text-red-600'} bg-slate-800 rounded-full py-2 px-4 text-sm">
        ...
        <div class="p-8 flex justify-end items-center">
            <form method="post" action="?/updateUser" use:enhance={submitBlock}>
                <button class="text-gray-400 py-2 hover:text-white">{user.enabled ? 'Disable user' : 'Enable user'}</button>
