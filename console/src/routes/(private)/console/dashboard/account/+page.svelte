...
<div class="w-full flex flex-col items-center mt-16">
    ...
    <div class="grid grid-cols-3 gap-2">
        ...
        <form class="contents" action="?/update" method="post" use:enhance={submit}>
            ...
            <code class="ml-2 w-64 text-sm h-full flex items-center text-red-600">{form.name ? form.name.message : ''}</code>
            ...
            <code class="ml-2 w-64 text-sm h-full flex items-center text-red-600">{form.email ? form.email.message : ''}</code>
            
            <!-- OLD PASSWORD -->
            <label for="oldPassword" class="h-11 w-full flex items-center pr-8">Old password</label>
            <div class="w-64 flex items-center">
                <Input id="oldPassword" name="oldPassword" type="password" placeholder="******" bind:value={oldPassword}/>
            </div>
            <code class="ml-2 w-64 text-sm h-full flex items-center text-red-600">{form.oldPassword ? form.oldPassword.message : ''}</code>
            
            <!-- NEW PASSWORD -->
            <label for="newPassword" class="h-11 w-full flex items-center pr-8">New password</label>
            <div class="w-64 flex items-center">
                <Input id="newPassword" name="newPassword" type="password" placeholder="******" bind:value={newPassword}/>
            </div>
            <code class="ml-2 w-64 text-sm h-full flex items-center text-red-600">{form.newPassword ? form.newPassword.message : ''}</code>
            
            <!-- REPEAT NEW PASSWORD -->
            <label for="re_new_password" class="h-11 w-full flex items-center pr-8 ">Repeat new password</label>
            <div class="w-64 flex items-center">
                <Input id="re_newPassword" name="re_newPassword" type="password" placeholder="******" bind:value={re_newPassword}/>
            </div>
            <code class="ml-2 w-64 text-sm h-full flex items-center text-red-600">{form.re_newPassword ? form.re_newPassword.message : ''}</code>
